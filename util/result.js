exports.resultData = function (data = null, status = 200, msg = "成功") {
  return {
...
// 前端没有传但后端定义了的参数过滤掉
exports.mergeExistingProperties = function (
  source,
  outValue = [undefined],
  outKey = [],
) {
  ...
  for (const key in source) {
    ...
    // 需要判断空数组
    if (
      outValue.some((value) => Array.isArray(value) && value.length === 0) &&
      source[key].length === 0
    ) {
      continue;
    }
